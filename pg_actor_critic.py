...
class PolicyGradientActorCritic(object):
  ...
  def create_variables(self):
    ...
    with tf.name_scope("model_inputs"):
      ...
      self.states_2 = tf.placeholder(tf.float32, (None, self.state_dim2), name="states2")

    # rollout action based on current policy
    with tf.name_scope("predict_actions"):
      # initialize actor-critic network
     
      with tf.variable_scope("share_network"):
        self.share_outputs = self.share_network(self.states)   
      with tf.variable_scope("actor_network"):
        self.policy_outputs = self.actor_network(self.share_outputs)
      with tf.variable_scope("critic_network"):
        self.value_outputs = self.critic_network(self.share_outputs)
    ...
    share_network_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="share_network")
    ...
    with tf.name_scope("compute_pg_gradients"):
      ...
      with tf.variable_scope("share_network", reuse=True):
        self.state_in2 =  self.share_network( self.states)
      with tf.variable_scope("actor_network", reuse=True):

        self.logprobs =   self.actor_network( self.state_in2 )

      with tf.variable_scope("critic_network", reuse=True):
        self.estimated_values = self.critic_network(  self.state_in2 )
      ...
      self.share_actor_gradients = self.optimizer.compute_gradients(self.actor_loss, share_network_variables)
      ...
      for i, (grad, var) in enumerate(self.share_actor_gradients):
        if grad is not None:
          self.share_actor_gradients[i] = (grad * self.advantages, var)
      ...
      self.share_critic_gradients  =  self.optimizer.compute_gradients( self.critic_loss, share_network_variables)

      # collect all gradients
      self.gradients = self.actor_gradients + self.critic_gradients + self.share_critic_gradients 
